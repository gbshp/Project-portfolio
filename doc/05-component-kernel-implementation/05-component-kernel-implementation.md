...
For the implementation, we would need three different kinds of structures. An int data structure to hold the number of qubits in the circuit; An array to represent complex numbers in java and the calculations. Finally, a List to store the gates (the mathematical operations) that are to be carried out on the qubits.
